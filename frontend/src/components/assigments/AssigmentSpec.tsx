...
function AssigmentSpec() {
  ...
  const [isSolutionChecked, setIsSolutionChecked] = useState<
    boolean | undefined
  >(undefined)
  ...
  useEffect(() => {
    postgresqlDatabase
      .get(
        `/solution/getUncheckedSolutionByUserAssignmentGroup/${userState.userId}/${idAssigment}/${id}`
      )
      .then((response) => {
      ...
      .finally(() => {
        postgresqlDatabase
          .get(
            `/solution/getCheckedSolutionByUserAssignmentGroup/${userState.userId}/${idAssigment}/${id}`
          )
          .then((response) => {
            if (response.data.id !== null) {
              setIsLoading(false)
