  ...
  return (
    <div className='ml-[7.5%]'>
      <Link to={idGroup!==undefined ? `/group/${idGroup}/assignment/${assignment.id}`: "#"} onClick={() => {handleAssignmentClick && handleAssignmentClick(assignment)}} state={optionalUserId}
            className="flex flex-row mt-4 border-border_gray border h-16 rounded-lg font-lato text-xl items-center  w-[900px] bg-white">
        ...
        <div className="mr-10 text-right font-semibold text-[28px] basis-1/5">{(resultPoints !== undefined) ? resultPoints : null} {"/ " + assignment.maxPoints}</div>
      </Link>
